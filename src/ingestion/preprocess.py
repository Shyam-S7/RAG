...
class TextCleaner:
    ...
    def clean(text: str) -> str:
        ...
        # Normalize unicode characters could go here
        # Remove excessive whitespace
        text = re.sub(r'\s+', ' ', text).strip()
        return text

class DomainDetector:
    """Detects technical domain using keywords."""
    
    KEYWORDS = {
        Domain.PROGRAMMING: ["python", "java", "c++", "function", "class", "import", "def ", "return"],
        Domain.DSA: ["algorithm", "complexity", "big o", "tree", "graph", "sorting", "dfs", "bfs"],
        Domain.SYSTEM_DESIGN: ["scalability", "load balancer", "database", "sharding", "cap theorem", "microservices"],
        Domain.IOT: ["sensor", "arduino", "raspberry pi", "mqtt", "esp32", "gpio", "voltage"],
        Domain.WEB_DEV: ["http", "api", "rest", "react", "html", "css", "json", "endpoint"],
        Domain.ML_AI: ["neural network", "transformer", "pytorch", "training", "inference", "loss function"],
        Domain.GEN_AI: ["llm", "generative", "gpt", "bert", "diffusion", "rag", "prompt engineering", "hallucination"],
        Domain.DATA_SCIENCE: ["dataframe", "pandas", "visualization", "statistics", "outlier", "regression"]
    }
    ...
    def detect(text: str) -> Domain:
        ...
        # Heuristic: If Programming > 0 and others are low, prefer Programming
        # But for now, simple max score
...
class MetadataMerger:
    ...
    @staticmethod
    def merge(doc_metadata: Dict[str, Any], extra_metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
...
class Preprocessor:
    ...
    def process_file(self, file_path: str) -> List[LangchainDocument]:
        ...
        try:
            ...
            logger.info(f"Successfully processed {file_path}: {len(final_chunks)} chunks created.")
            return final_chunks
